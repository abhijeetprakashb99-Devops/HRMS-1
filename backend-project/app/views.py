...
from .models import Employee, HRProfile, Payroll, TeamLeader, TeamAssignment, ProjectAssignment, Announcement, Attendance, ProjectTask, ProjectMilestone, ProjectDiscussion, LeaveApply, LeaveApproval, MonthlyAttendanceSummary, AttendanceApproval, PayrollDeduction, SalaryProcessing, AttendanceCheckLog
...
from .models import HRProfile, Employee, TeamLeader, ProjectManager
...
def login_view(request):
    if request.method == "POST":
        ...
        # =============================
        # 4️⃣ PROJECT MANAGER LOGIN ✅
        # =============================
        if role == "pm":
            try:
                # 🔹 personal_email se login
                pm = ProjectManager.objects.get(personal_email=email)

                # 🔹 password check (hashed)
                if check_password(password, pm.password):
                    request.session["pm_id"] = pm.id
                    request.session["role"] = "pm"
                    request.session["pm_name"] = pm.name

                    messages.success(request, f"Welcome Project Manager {pm.name}!")
...
def create_project_manager(request):
    if request.method == "POST":
        ...
        if password != confirm_password:
            messages.error(request, "Password and Confirm Password do not match")
            return render(request, "app/hr/project-manager.html")

        # Duplicate Check
        if ProjectManager.objects.filter(employee_id=employee_id).exists():
            messages.error(request, f"Employee ID '{employee_id}' is already used")
            return render(request, "app/hr/project-manager.html")

        if ProjectManager.objects.filter(official_email=official_email).exists():
            messages.error(request, f"Official Email '{official_email}' is already used")
            return render(request, "app/hr/project-manager.html")

        if ProjectManager.objects.filter(personal_email=personal_email).exists():
            messages.error(request, f"Personal Email '{personal_email}' is already used")
            return render(request, "app/hr/project-manager.html")
        ...
        try:
            ...
            # Prepare Email
            subject = "Project Manager Access Credentials"
            message = f"""
Hello {name},

Your Project Manager profile has been created successfully.
...
Please use your OFFICIAL EMAIL for system login.
These credentials are sent to your PERSONAL EMAIL for security From Kavya Infoweb Pvt Ltd.
...
"""
            ...
            messages.success(
                request,
                "Project Manager Profile Created Successfully & Access Credential Sent to Personal Email"
            )

        except IntegrityError:
            messages.error(request, "Duplicate entry detected. Please check Employee ID or Emails.")
        except Exception as e:
            messages.error(request, f"Profile created but failed to send email: {str(e)}")

        # Reload same page after submission
        return render(request, "app/hr/project-manager.html")
...
def pm_dashboard(request):
    ...
    # PROJECT STATS
    # =========================
    total_projects = ProjectAssignment.objects.count()
    completed_projects = ProjectAssignment.objects.filter(is_completed=True).count()
    active_projects = ProjectAssignment.objects.filter(is_completed=False).count()

    # =========================
    # TASK STATS
    # =========================
    total_tasks = ProjectTask.objects.count()
    completed_tasks = ProjectTask.objects.filter(task_status='completed').count()
    pending_tasks = ProjectTask.objects.exclude(task_status='completed').count()

    # =========================
    # TEAM STATS
    # =========================
    total_team_leads = TeamLeader.objects.count()
    total_employees = Employee.objects.count()
    ...
    return render(request, "app/pm/dashboard.html", context)


def project_manager_logout(request):
    """
    Project Manager logout confirmation page (GET request)
    """
    pm_id = request.session.get('pm_id')
    ...
    try:
        ...
        messages.error(request, "Project Manager not found.")
        return redirect('pm-login')

    context = {
        'user_info': user_info
    }
    return render(request, 'app/logout_confirmations.html', context)


def project_manager_confirm_logout(request):
    """
    Project Manager actual logout (POST request)
    """
    if request.method == "POST":
        action = request.POST.get('action')
        if action == 'confirm_logout':
            request.session.flush()
            messages.success(request, "You have been logged out successfully.")
        return redirect('login')  # Redirect PM login
    else:
        return redirect('project-manager-dashboard') 
    

from django.shortcuts import render, get_object_or_404
from .models import (ProjectManager,TeamLeader,TeamAssignment,ProjectAssignment,Employee )


def project_manager_profile(request, pm_id):
    pm = get_object_or_404(ProjectManager, employee_id=pm_id)
    # ================= COUNTS =================
    total_team_leads = TeamLeader.objects.count()
    total_projects = ProjectAssignment.objects.count()
    total_team_members = TeamAssignment.objects.values(
        'employee'
    ).distinct().count()
    total_employees = Employee.objects.count()

    # Team Leads list (display purpose)
    team_leads = TeamLeader.objects.select_related('employee')
    ...
    return render(request, "app/pm/pm_profile.html", context)
...
def client_management(request):
    ...
    total_projects = clients.count()  # assuming 1 project per client
    ongoing_projects = clients.filter(project_status='Ongoing').count()
    completed_projects = clients.filter(project_status='Completed').count()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('clients')  # reload page after save
    else:
        form = ClientForm()
    ...
    return render(request, 'app/pm/client.html', context)
...
from django.shortcuts import render, get_object_or_404
from .models import ProjectAssignment, TeamLeader, Employee

# ----------------- Project Page -----------------
def pm_projects(request):
    projects = ProjectAssignment.objects.select_related('team_leader').prefetch_related('team_members')
    context = {
        "projects": projects,
        "total_projects": ProjectAssignment.objects.count(),
        "total_team_leads": TeamLeader.objects.count(),
    ...
    return render(request, "app/pm/project.html", context)

# ----------------- Team Lead Modal -----------------
def team_lead_profile(request, lead_id):
    lead = get_object_or_404(TeamLeader, id=lead_id)
    members = lead.projectassignment_set.first().team_members.all() if lead.projectassignment_set.exists() else []
    return render(request, "app/pm/model_team_lead.html", {
        "lead": lead,
        "members": members
    })

def team_lead_details(request, lead_id):
    lead = get_object_or_404(TeamLeader, id=lead_id)
    ...
    return render(request, "app/pm/team-lead-details.html", {
        "lead": lead,
        "projects": projects,
    })
...
def pm_teams(request):
    teams = TeamLeader.objects.select_related('employee') \
        .prefetch_related('projectassignment_set__team_members')
    ...
    return render(request, "app/pm/teams.html", context)


def team_view(request, id):
    team = get_object_or_404(TeamLeader, id=id)
    projects = ProjectAssignment.objects.filter(team_leader=team)
    return render(request, "app/pm/ajax/team_view.html", {
        "team": team,
        "projects": projects
    })
...
def team_edit(request, id):
    ...
    if request.method == "POST":
        team.employee.first_name = request.POST['first_name']
        team.employee.last_name = request.POST['last_name']
        team.employee.email = request.POST['email']
        team.employee.save()
        return redirect('pm-teams')

    return render(request, "app/pm/ajax/team_edit.html", {"team": team})
...
def pm_leave_manage(request):

    leave_requests = LeaveApply.objects.select_related(
        'employee'
    ).order_by('-applied_at')
    ...
    context = {
        ...
        'tl_approved': leave_requests.filter(tl_approved=True).count(),
        'hr_approved': leave_requests.filter(status='approved').count(),
    }

    return render(request, 'app/pm/leave-manage.html', context)


# ✅ PM Leave View (SINGLE LEAVE DETAIL)
def pm_leave_view(request, id):
    leave = get_object_or_404(LeaveApply, id=id)
...
def pm_dashboard(request):
    pm_id = request.session.get('pm_id')
    if not pm_id:
        messages.error(request, "Please login to access Project Manager dashboard.")
        return redirect('login')
    ...
    context = {
        "pm_name": pm.name,
        "pm_email": pm.official_email,
        "pm_id": pm.employee_id,
    }
...
from django.db.models import Count, Q
from .models import ProjectAssignment, TeamLeader, TeamAssignment, Employee, LeaveApply

def pm_reports(request):
    # ================= Dashboard Stats =================
    total_projects = ProjectAssignment.objects.count()
    ongoing_projects = ProjectAssignment.objects.filter(is_completed=False).count()
    completed_projects = ProjectAssignment.objects.filter(is_completed=True).count()

    total_team_leads = TeamLeader.objects.count()
    total_employees = Employee.objects.count()
    total_team_members = TeamAssignment.objects.values('employee').distinct().count()

    # ================= Leave Stats =================
    total_leave_requests = LeaveApply.objects.count()
    pending_leaves = LeaveApply.objects.filter(status='pending').count()
    tl_approved_leaves = LeaveApply.objects.filter(status='tl_approved').count()
    hr_approved_leaves = LeaveApply.objects.filter(status='hr_approved').count()
    rejected_leaves = LeaveApply.objects.filter(status='rejected').count()

    # ================= Latest Projects & Leaves =================
    latest_projects = ProjectAssignment.objects.order_by('-created_at')[:5]
    latest_leaves = LeaveApply.objects.order_by('-applied_at')[:5]

    # ================= Team Leads with Projects =================
    team_leads = TeamLeader.objects.prefetch_related('projectassignment_set__team_members')
    ...
    context = {
        ...
        "pending_leaves": pending_leaves,
        "tl_approved_leaves": tl_approved_leaves,
    ...
    return render(request, "app/pm/reports.html", context)
